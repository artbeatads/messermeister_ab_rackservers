...
class AEPostprocSftp extends AEAbstractPostproc
{
	...
	public function processPart($absolute_filename)
	{
		...
		if(empty($directory)) $directory	= $config->get('engine.postproc.sftp.initial_directory', '');
	...
	public function delete($path)
	{
		...
		AEUtilLogger::WriteLog(_AE_LOG_DEBUG, __CLASS__.'::delete() -- Connecting to remote SFTP');
	...
	public function downloadToFile($remotePath, $localFile, $fromOffset = null, $length = null)
	{
		...
		if ($localfp === false)
		{
			$this->setWarning("Could not open local file $localFile for reading");
			@fclose($fp);
		...
		if($res === false)
		{
			$this->setWarning( 'Downloading ' . $remotePath . ' has failed because the source file is unreadable.');
	 ...
	 * @return  boolean
	 */
	private function _sftp_chdir($dir, &$sftphandle)
	{
