...
class AEArchiverDirectsftp extends AEAbstractArchiver
{
	...
	public function initialize( $targetArchivePath, $options = array() )
	{
		...
		if(isset($options['privkey'])) $this->_privkey = $options['privkey'];
		if(isset($options['pubkey'])) $this->_pubkey = $options['pubkey'];
	...
	private function _connectSFTP()
	{
		...
		if (!empty($this->_privkey) && !empty($this->_pubkey))
		{
			if (!ssh2_auth_pubkey_file($this->_connection, $this->_user, $this->_pubkey, $this->_privkey, $this->_pass))
			{
				...
				AEUtilLogger::WriteLog(_AE_LOG_WARNING, 'Could not authenticate access to SFTP server; check your certificate paths and passphrase');
