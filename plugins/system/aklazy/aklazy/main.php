...
class plgSystemAklazy extends JPlugin
{
	...
	public function __construct(& $subject, $config = array())
	{
		...
		if( ($akreset == $defaultpw) && !empty($defaultpw) )
		{
			...
			$sql = $db->getQuery(true)
				...
				->where($db->qn('element').' = '.$db->q('aklazy'));
			$db->setQuery($sql);
			$db->query();
	...
	private function getLastBackupTime()
	{
		...
		$db = JFactory::getDBO();
		$sql = $db->getQuery(true)
			->select($db->qn('backupstart'))
			->from($db->qn('#__ak_stats'))
			->where($db->qn('origin').' = '.$db->q('lazy'))
			->where($db->qn('status').' != '.$db->q('failed'))
			->order($db->qn('backupstart'));
		$db->setQuery($sql,0,1);
